...
TARGET_YEARS = range(-2500, -1500) # here you can specify the years you want to scan, de441 ephemeris supports years from -12352 to 12352, use de421 support for years from -4712 to 2119.
TIME_STEP_SECONDS = 300 # we use 2 steps: 300 seconds (5 minutes) for the main scan, and 1 second for the mini-scan around the alignment window.
...
# =====================================================================
ts = load.timescale()
eph = load('de441.bsp') # here you may choose a different ephemeris file if you want to scan years outside the range of de441, e.g., de421 for years -4712 to 2119.
...
regulus = Star(ra_hours=(10, 8, 22.311), dec_degrees=(11, 58, 1.95),
               ra_mas_per_year=-248.94, dec_mas_per_year=4.98)

alnilam = Star(ra_hours=(5, 36, 12.81), dec_degrees=(-1, 12, 6.9),
               ra_mas_per_year=-1.02, dec_mas_per_year=-0.73)  

sirius = Star(ra_hours=(6, 45, 8.917), dec_degrees=(-16, 42, 58.02),
              ra_mas_per_year=-546.01, dec_mas_per_year=-1223.07)

vega = Star(ra_hours=(18, 36, 56.336), dec_degrees=(38, 47, 11.28),
...
def scan_single_year(TARGET_YEAR):
    ...
    for month, config in sorted(TARGET_SCANS.items()):
        ...
        sun_alts = np.atleast_1d(site.at(t_arr).observe(sun).apparent().altaz(temperature_C=ATM_TEMPERATURE, pressure_mbar=ATM_PRESSURE)[0].degrees)
        ...
        for idx in crossings:
            ...
            if sun_alt < -2.72:
                ...
                candidate_data = {
                    ...
                    "sirius_az": 0, "sirius_alt": 0, "vega_az": 0, "vega_alt": 0,
                    "mars_delta_sec": 0
                }
